...
func (h anilistAuthHandler) get(w http.ResponseWriter, r *http.Request) {
	...
	h.encoder.StatusResponse(w, http.StatusOK, anilistConfig{
...
// startOauth begins the AniList OAuth flow.
// It stores the config + state in the database, then returns the authorization URL
// for the frontend to open in a new window.
// NOTE: the auth URL is built BEFORE calling Store(), because Store() encrypts the
// ClientID in-place — using it afterwards would produce a garbled client_id parameter.
...
// redirects the browser with ?code=...&state=...
// The frontend calls this API endpoint, persists the token, then notifies the
// opener (settings page) via postMessage and closes the window.
